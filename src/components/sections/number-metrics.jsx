...
const NumberMetrics = ({ metrics = [] }) => {
  return (
    <section className="relative flex min-h-[700px] flex-col overflow-hidden py-6 md:min-h-[440px]">
      ...
      <div className="relative mx-auto my-auto grid max-w-6xl grid-cols-1 gap-12 px-8 md:my-0 md:mt-auto md:grid-cols-4 md:gap-x-16 md:px-0">
        {metrics.map((metric, index) => (
          <div key={index} className="space-y-2 text-center lg:col-span-1">
            ...
            <p className="max-w-[160px] text-xl md:max-w-[180px] md:text-2xl">
              {metric.label}
            </p>
          </div>
        ))}
